...
addEvent(document, 'readystatechange', function () {
    ...
    function updateCart() {
        ...
        document.querySelectorAll("#cart span.total")[0].innerHTML = "Total: $" + total;
    }

    // This function is only called when adding 1 item, otherwise updateCartItem(item) is called
    function addCartItem(item, id) {
        var original = document.querySelectorAll("#products ul li[id='" + id + "'] input");/** Take the <input> from product li with specific id **/
        original[0].setAttribute('data-id', id); /* give <input> a data id (the id is the same id as the li id) */
    ...
    function onDrop(event) {
        ...
        var original = document.querySelectorAll("#cart ul li input[data-id='" + id + "']"); /**ADDED THIS myself **/
